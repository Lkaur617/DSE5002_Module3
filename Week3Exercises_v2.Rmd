...
reorder_name<-function(last_first){
  #your code here
  name_parts <- str_split(str_replace_all(last_first, ",", ""), " ")[[1]]
...
powers_df <- function(x) {
  # Create the data frame with columns: x, x^2, and sqrt(x)
  df <- data.frame(
    x = x,
    x_squared = x^2,
    sqrt_x = sqrt(x)
...
calculate_y <- function(x) {
  # Apply the condition using ifelse()
...
next_power_of_two <- function(x) {
  # Start with the first power of two (2^0 = 1)
  power_of_two <- 1
  
  # While loop to find the first power of two greater than x
...
phone_a <- str_extract(a, pattern)
phone_b <- str_extract(b, pattern)
phone_c <- str_extract(c, pattern)
...
# Extract domains
domain_d <- str_extract(d, pattern)
domain_e <- str_extract(e, pattern)
domain_f <- str_extract(f, pattern)
